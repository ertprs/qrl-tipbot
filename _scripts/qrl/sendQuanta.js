...
async function qrlWallet(args) {
  ...
  // no args passed, get the defaults from the config and create a wallet
    ...
    const { stdout, stderr } = await exec(`curl -s -XPOST http://127.0.0.1:5359/api/AddNewAddressWithSlaves -d'{  "height": ${arguments[0]}, "number_of_slaves": ${arguments[1]}, "hash_function": "${arguments[2]}"}'`);
    if (stderr) {
      console.error(`error: ${stderr}`);
    }
    console.log(`qrlWallet: ${stdout}`);
