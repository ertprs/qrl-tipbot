...
const botDiscordID = () => {
 return new Promise((resolve) => {
    rl.question(chalk.cyan.bold('Provide Bot Discord ID: '), (bot_Discord_ID) => {
...
const botDiscordUserName = () => {
  return new Promise((resolve) => {
    rl.question(chalk.cyan.bold('Provide bot Discord User Name: '), (bot_Discord_Username) => {
...
const walletEncryptionPass = () => {
 return new Promise((resolve) => {
    rl.question(chalk.cyan.bold('Provide wallet Encryption password: '), (wallet_encryption_pass) => {
...
main().then(function() {
  ...
  faucetWalletPromise.then(function(faucet_Wallet_Info) {
    ...
    const faucetSecretKeyPromise = secretKey(walletInfoArray[0].faucetInfo.address);
    faucetSecretKeyPromise.then(function(faucetSecrets) {
      const faucetSec = JSON.stringify(JSON.parse(faucetSecrets));
      console.log(chalk.cyan.bold('Faucet Secret Keys: ') + chalk.red.bold(faucetSec));
        // create hold wallet
        holdWalletPromise.then(function(hold_Wallet_Info) {
          ...
          holdSecretKeyPromise.then(function(holdSecrets) {
            ...
            console.log(chalk.cyan.bold('Hold Secret Keys: ') + chalk.red.bold(holdSec));
            ...
            botWalletPromise.then(function(address) {
              ...
              console.log(chalk.cyan.bold('\nBot Pub Address: ') + chalk.blue.bold(JSON.stringify(walletInfoArray[2].botInfo.wallet_pub)));
              const botSecretKeyPromise = secretKey(walletInfoArray[2].botInfo.wallet_pub);
              botSecretKeyPromise.then(function(botSecrets) {
                const botSec = JSON.stringify(JSON.parse(botSecrets));
                console.log(chalk.cyan.bold('Bot Secret Keys: ') + chalk.red.bold(botSec));
                ...
                AddUserPromise.then(function() {
                  ...
                  // console.log('Results:');
                  console.log(chalk.cyan.bold('Wallet encryption key: ') + chalk.red.bold(infoArray[2].wallet_encryption_pass) + '\n');
                  process.exit();
                });
              });
            });
