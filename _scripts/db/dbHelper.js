...
async function GetAllUserInfo(args) {
  return new Promise(resolve => {
    ...
    const getAllInfoSearch = 'SELECT wallets.* , users.*, ' + service + '_users.*, users_info.*, users_agree.*, future_tips.* FROM wallets, users, ' + service + '_users, users_info, users_agree, future_tips WHERE users.id = wallets.user_id AND users.' + service + '_user_id = ' + service + '_users.id AND users.id = users_info.user_id AND ' + service + '_users.' + service + '_id = "' + service_id + '" AND users.id = users_info.user_id AND future_tips.user_id = "' + service_id + '"';
    
    //     const getAllInfoSearch = 'SELECT wallets.wallet_pub AS wallet_pub, wallets.wallet_bal AS wallet_bal, sers.id AS user_id, ' + service + '_users.user_name AS user_name, users_info.opt_out AS opt_out,  users_info.optout_date AS optout_date FROM wallets, users, ' + service + '_users, users_info WHERE users.id = wallets.user_id AND users.' + service + '_user_id = ' + service + '_users.id  AND  users.id = users_info.user_id  AND ' + service + '_users.' + service + '_id = "' + service_id + '"';
