      ...
      //  console.log('tippedUserInfo: ' + JSON.stringify(tippedUserInfo));
        ...
        wallet.sendQuanta(tipToInfo).then(function(sendData) {
          ...
          //  console.log('transactionDBresp: ' + JSON.stringify(transactionDBresp));
            ...
            message.channel.stopTyping(true);
            if (tipUserCount > 1) {
              ReplyMessage('You\'ve sent a `' + toQuanta(givenTip) + ' QRL` tip was sent to ' + tippedUserIDs + ',' + futureTippedUserIDs + ' each. Thanks for using the tipbot!\n*All tips are on-chain, and will take some time to process.*');
            }
            else {
              ReplyMessage('You\'ve sent a `' + toQuanta(givenTip) + ' QRL` tip was sent to ' + tippedUserIDs + ',' + futureTippedUserIDs + ' Thanks for using the tipbot!\n*All tips are on-chain, and will take some time to process.*');
            }
